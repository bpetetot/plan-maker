...
describe('planStore undo/redo', () => {
  it('records one history step per editPlan and undoes/redoes it', () => {
    editPlan((p) => addRoomProfile(p, 'Kitchen', 10, 10)[0]);
    editPlan((p) => addRoomProfile(p, 'Bedroom', 20, 20)[0]);
    expect(Object.keys(plan().roomProfiles)).toHaveLength(2);

    undo();
    expect(Object.keys(plan().roomProfiles)).toHaveLength(1);
    undo();
    expect(Object.keys(plan().roomProfiles)).toHaveLength(0);
    redo();
    expect(Object.keys(plan().roomProfiles)).toHaveLength(1);
  });

  it('editPlan returns the plan it landed', () => {
    const landed = editPlan((p) => addRoomProfile(p, 'Kitchen', 10, 10)[0]);
    expect(landed).toBe(plan());
    expect(Object.values(landed.roomProfiles)[0].name).toBe('Kitchen');
  });
  ...
  it('collapses every aim of one Edit into a single undo step', () => {
    editPlan((p) => addRoomProfile(p, 'Start', 0, 0)[0]);

    const edit = beginEdit();
    edit.aim(addRoomProfile(plan(), 'Drag 1', 1, 1)[0]);
    edit.aim(addRoomProfile(plan(), 'Drag 2', 2, 2)[0]);
    edit.land(addRoomProfile(plan(), 'Drag 3', 3, 3)[0]);

    expect(Object.keys(plan().roomProfiles)).toHaveLength(4);
    undo();
    expect(Object.keys(plan().roomProfiles)).toHaveLength(1);
    redo();
    expect(Object.keys(plan().roomProfiles)).toHaveLength(4);
  ...
  it('marks the store while an Edit is open and clears it on landing', () => {
    const edit = beginEdit();
    edit.aim(addRoomProfile(plan(), 'Aimed', 0, 0)[0]);
  ...
  it('lands an abandoned Edit when the next one opens, and deadens its handle', () => {
    ...
    abandoned.aim(addRoomProfile(plan(), 'Aimed', 0, 0)[0]);
    ...
    abandoned.aim(addRoomProfile(plan(), 'Ghost', 5, 5)[0]);
    abandoned.land(addRoomProfile(plan(), 'Ghost', 5, 5)[0]);
    expect(Object.keys(plan().roomProfiles)).toHaveLength(1);

    next.land(addRoomProfile(plan(), 'Real', 9, 9)[0]);
    expect(Object.keys(plan().roomProfiles)).toHaveLength(2);
  });

  it('lands an abandoned Edit before a plain one, so neither folds into the other', () => {
    const abandoned = beginEdit();
    abandoned.aim(addRoomProfile(plan(), 'Dragged', 0, 0)[0]);

    editPlan((p) => addRoomProfile(p, 'Typed', 5, 5)[0]);
    expect(usePlanStore.getState().editOpen).toBe(false);
    expect(Object.keys(plan().roomProfiles)).toHaveLength(2);
  ...
  it('caps history at 100 steps', () => {
    for (let i = 0; i < 130; i++) {
      editPlan((p) => addRoomProfile(p, `L${i}`, i, i)[0]);
    }
